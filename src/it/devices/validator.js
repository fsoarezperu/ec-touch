  ...
  return new Promise( async function(resolve, reject) {
    ...
    try {
      ...
      var step2=await ssp.negociate_encryption(validator_address);
              if (step2=="OK") {
                //aqui ya se negocio satisfactoriamete la comunicacion segura.
                  var step3= await validatorpoll2(validator_address);
                  console.log("STEP3 trae como respusta de validatorpool2:"+step3);
                  //si no existe bolsa.
                  console.log("existe_bolsa:"+ existe_bolsa);
                  if (existe_bolsa==false) {
                      await os.new_unlock_cashbox();
                    await validator_poll_loop(validator_address);
                    return reject("No hay bolsa en la caja");
                  }
